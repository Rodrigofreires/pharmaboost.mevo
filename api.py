...
app = FastAPI(
    ...
    version="4.1.0" # Versão com Agente Padrão Mevofarma
)
...
@app.post("/process-with-mevofarma-standard", tags=["Processador Padrão Mevofarma"])
async def process_with_mevofarma_standard(
    ...
    usando o novo agente PADRÃO MEVOFARMA.
    """
    try:
    ...
    async def event_stream():
        ...
        try:
            ...
            yield await send_event("log", {"message": f"Iniciando processamento com o AGENTE PADRÃO MEVOFARMA para {total_bulas} SKUs...", "type": "info"})
            ...
            for i, ((bula_filename, bula_bytes), sku) in enumerate(zip(bulas_data, sku_list)):
                ...
                try:
                    ...
                    product_info_simulado = {"bula_text": texto_da_bula}

                    yield await send_event("log", {"message": f"{log_prefix} Enviando para o Agente Padrão Mevofarma...", "type": "info"})

                    # Chama a NOVA pipeline
                    mevofarma_generator = use_cases.run_mevofarma_pipeline_stream(
                        product_name=nome_produto,
                        product_info=product_info_simulado
                    )

                    final_content_data = None
                    async for event_chunk in mevofarma_generator:
                        yield event_chunk
                    ...
                    if final_content_data:
                        ...
                        yield await send_event("log", {"message": f"{log_prefix} Conteúdo gerado com o Padrão Mevofarma. Aguardando sua revisão.", "type": "success"})
                    else:
                         yield await send_event("log", {"message": f"{log_prefix} ERRO: O agente Padrão Mevofarma não retornou um resultado final.", "type": "error"})
